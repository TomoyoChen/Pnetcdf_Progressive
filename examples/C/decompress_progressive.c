...
static int create_output_file(const char *input_path, const char *output_path, int ncid_in,
                              ...
                              int *ncid_out_ptr, int *flds_varid_out_ptr)
 {
    ...
    /* Use a plain non-chunking handle for metadata/coordinate copies.
     * The chunked progressive handle is reserved for FLDS decompression. */
    CHECK_ERR(ncmpi_open(MPI_COMM_WORLD, input_path, NC_NOWRITE,
                         MPI_INFO_NULL, &ncid_meta));
     
     /* Get input variable IDs */
     int x_varid_in, y_varid_in, time_varid_in;
     int lat_varid_in, lon_varid_in, proj_varid_in, flds_varid_in;
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "x", &x_varid_in));
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "y", &y_varid_in));
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "time", &time_varid_in));
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "lat", &lat_varid_in));
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "lon", &lon_varid_in));
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "lambert_conformal_conic", &proj_varid_in));
    CHECK_ERR(ncmpi_inq_varid(ncid_meta, "FLDS", &flds_varid_in));
     ...
     /* Copy attributes */
    copy_var_atts(ncid_meta, x_varid_in, ncid_out, x_varid_out);
    copy_var_atts(ncid_meta, y_varid_in, ncid_out, y_varid_out);
    copy_var_atts(ncid_meta, time_varid_in, ncid_out, time_varid_out);
    copy_var_atts(ncid_meta, lat_varid_in, ncid_out, lat_varid_out);
    copy_var_atts(ncid_meta, lon_varid_in, ncid_out, lon_varid_out);
    copy_var_atts(ncid_meta, proj_varid_in, ncid_out, proj_varid_out);
    copy_var_atts(ncid_meta, flds_varid_in, ncid_out, flds_varid_out);
    copy_global_atts(ncid_meta, ncid_out);
 
     CHECK_ERR(ncmpi_enddef(ncid_out));
 
     /* Write coordinate variables (rank 0 only) */
    CHECK_ERR(ncmpi_begin_indep_data(ncid_meta));
     ...
     if (rank == 0) {
         ...
         short proj;
        MPI_Offset start_x[1] = {0}, count_x[1] = {x_len};
        MPI_Offset start_y[1] = {0}, count_y[1] = {y_len};
        MPI_Offset start_t[1] = {0}, count_t[1] = {time_len};
        MPI_Offset start_latlon[2] = {0, 0}, count_latlon[2] = {y_len, x_len};
        MPI_Offset start_proj[1] = {0};
 
        CHECK_ERR(ncmpi_get_vara_float(ncid_meta, x_varid_in, start_x, count_x, buf_x));
        CHECK_ERR(ncmpi_get_vara_float(ncid_meta, y_varid_in, start_y, count_y, buf_y));
        CHECK_ERR(ncmpi_get_vara_float(ncid_meta, time_varid_in, start_t, count_t, buf_t));
        CHECK_ERR(ncmpi_get_var1_short(ncid_meta, proj_varid_in, start_proj, &proj));
        CHECK_ERR(ncmpi_get_vara_float(ncid_meta, lat_varid_in, start_latlon, count_latlon, buf_latlon));
 
        CHECK_ERR(ncmpi_put_vara_float(ncid_out, x_varid_out, start_x, count_x, buf_x));
        CHECK_ERR(ncmpi_put_vara_float(ncid_out, y_varid_out, start_y, count_y, buf_y));
        CHECK_ERR(ncmpi_put_vara_float(ncid_out, time_varid_out, start_t, count_t, buf_t));
        CHECK_ERR(ncmpi_put_var1_short(ncid_out, proj_varid_out, start_proj, &proj));
        CHECK_ERR(ncmpi_put_vara_float(ncid_out, lat_varid_out, start_latlon, count_latlon, buf_latlon));
 
        CHECK_ERR(ncmpi_get_vara_float(ncid_meta, lon_varid_in, start_latlon, count_latlon, buf_latlon));
        CHECK_ERR(ncmpi_put_vara_float(ncid_out, lon_varid_out, start_latlon, count_latlon, buf_latlon));
    ...
    CHECK_ERR(ncmpi_end_indep_data(ncid_meta));
     CHECK_ERR(ncmpi_end_indep_data(ncid_out));
    CHECK_ERR(ncmpi_close(ncid_meta));
 ...
 int main(int argc, char **argv)
 {
     ...
     for (int eb_idx = 0; eb_idx < num_ebs; eb_idx++) {
       ...
       /* Open compressed input for this error bound (fresh cache state) */
        ...
        /* Tell ncchunkio to use progressive IPComp decompression on reads. */
        ...
        CHECK_ERR(create_output_file(input_path, output_path, ncid_in, time_len, y_len, x_len,
        ...
        if (rank == 0) {
            ...
            printf("Read mode: independent owner-only (parallel by chunk owner)\n");
         }
       /*
         * Independent owner-only get:
         * app-side owner map matches ncchunkio's total-chunk contiguous mapping,
         * so owners can read/decompress in parallel without global synchronization.
         */
         ...
         for (MPI_Offset t = 0; t < time_len; t += chunk_shape[0]) {
             ...
             for (MPI_Offset y = 0; y < y_len; y += chunk_shape[1]) {
                 ...
                 for (MPI_Offset x = 0; x < x_len; x += chunk_shape[2]) {
                     ...
                     int owner = get_chunk_owner(chunk_id, nprocs, total_chunks);
                    ...
                    if (owns_chunk) {
                        ...
                        CHECK_ERR(ncmpi_get_vara_float(ncid_in, flds_varid,
                                                       ...
                                                       chunk_ptr));
                        t_read += MPI_Wtime() - t0;
                        /* Safety cleanup: keep invalid payload deterministic. */
                        {
                            float out_fill = has_fill_value ? fill_value : NC_FILL_FLOAT;
                            for (size_t idx = 0; idx < chunk_elems; idx++) {
                                float v = chunk_ptr[idx];
                                if (isnan(v) || isinf(v)) {
                                    chunk_ptr[idx] = out_fill;
                                }
                            }
                        }
                    }
        ...
        CHECK_ERR(ncmpi_var_clear_decomp_error_bound(ncid_in, flds_varid));
