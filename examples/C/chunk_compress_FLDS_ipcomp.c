...
{
    char *help =
    "Usage: %s [-h | -q | -t | -c] [-k format] [-l layers] -i in_file -o out_file\n"
    "       [-h] Print help\n"
    ...
    "       [-l layers]: IPComp layers (default: 1)\n"
           ...
           int         layers)
{
    ...
    MPI_Info_set(info, "nc_chk_cown_ratio", "0");
    ...
    /* use IPComp filter */
    err = ncmpi_var_set_filter(ncid, varid, NC_FILTER_IPCOMP); ERR

    /* minimal IPComp attributes */
    level_progressive = layers;
    err = ncmpi_put_att_text(ncid, varid, "comp:algo", (MPI_Offset)6, "ipcomp"); ERR
    err = ncmpi_put_att_int (ncid, varid, "comp:layers", NC_INT, 1, &layers); ERR
    err = ncmpi_put_att_int (ncid, varid, "comp:level_progressive", NC_INT, 1, &level_progressive); ERR
    ...
    if (rank == 0) {
        ...
        printf("Compression filter:       IPComp (layers=%d)\n", layers);
...
int main(int argc, char** argv)
{
    ...
    int i, rank, kind=0, cmode=0, nerrs=0, parti, div_time, layers=1;
    ...
    while ((i = getopt(argc, argv, "hqtck:i:o:l:")) != EOF)
        switch(i) {
            case 'q': verbose = 0;
                      break;
    ...
    if (verbose && rank == 0) printf("%s: test compression using ELM's FLDS data (IPComp)\n",__FILE__);
    ...
    nerrs += pnetcdf_io(MPI_COMM_WORLD, in_path, out_path, cmode, div_time, parti, layers);
