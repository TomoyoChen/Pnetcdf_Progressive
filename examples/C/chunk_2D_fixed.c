...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * This example shows how to use the multi-variable chunking and compression 
 * features of PNetCDF to write multiple 3D record variables with different 
...
compress(MPI_Comm comm, char *filename, int cmode)
{
    ...
    /* define 3D variables of integer type with different compression settings */
    for (i=0; i<NVARS; i++) {
        ...
        /* Set variable-specific compression algorithms and parameters */
        if (i == 0) {
            /* var_0: Use SZ compression with custom error bounds */
            ...
            /* Set SZ relative error bound ratio */
            double sz_rel_ratio = 0.001;  /* 0.1% relative error */
            err = ncmpi_var_set_sz_rel_bound_ratio(ncid, varid[i], sz_rel_ratio);
            PNC_ERR("ncmpi_var_set_sz_rel_bound_ratio")

            if (verbose && rank == 0)
                printf("var_%d: SZ compression, abs_err=%.3f, rel_ratio=%.4f\n", 
...
decompress(MPI_Comm comm, char *filename)
{
    ...
    /* get chunking and compression parameters */
    for (i=0; i<nvars; i++) {
        ...
        if (verbose && rank == 0) {
            printf("var %s filter is %s", name,
                (filter == NC_FILTER_DEFLATE) ?
                "NC_FILTER_DEFLATE": (filter == NC_FILTER_SZ) ?
                "NC_FILTER_SZ" : "UNKNOWN");

            /* Get and display compression-specific parameters */
            if (filter == NC_FILTER_SZ) {
                double sz_abs_err, sz_rel_ratio;
                err = ncmpi_var_get_sz_abs_err_bound(ncid, varid[i], &sz_abs_err);
                if (err == NC_NOERR) {
                    err = ncmpi_var_get_sz_rel_bound_ratio(ncid, varid[i], &sz_rel_ratio);
                    if (err == NC_NOERR) {
                        printf(", abs_err=%.3f, rel_ratio=%.4f", sz_abs_err, sz_rel_ratio);
                    }
                }
            }
            else if (filter == NC_FILTER_DEFLATE) {
                int zlib_level;
                err = ncmpi_var_get_zlib_level(ncid, varid[i], &zlib_level);
                if (err == NC_NOERR) {
                    printf(", level=%d", zlib_level);
                }
            }
            printf("\n");
        }
